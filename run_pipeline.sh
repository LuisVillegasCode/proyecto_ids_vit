...
for dia in "${!archivos[@]}"; do
    ...
    aws s3 cp \
        "s3://cse-cic-ids2018/Original Network Traffic and Log data/${dia}/pcap.zip" \
        "data/downloads/${dia}.zip" \
        --no-sign-request
    ...
    find "data/downloads/${dia}" -type f -print0 |
    while IFS= read -r -d '' file; do
        ...
        mv -- \
            "$file" \
            "data/raw/chunks/${prefijo}_${clean_name}.pcap"
    ...
    # ----------------------------------------------------
    # E. Fragmentar archivos grandes
    # ----------------------------------------------------

    echo "[*] Buscando archivos > 1GB..."

    find data/raw/chunks \
        -type f \
        -name "${prefijo}_*.pcap" \
        -size +1G \
        -print0 |
...
echo "[🚀] SOLO DEBERÍAN QUEDAR LOS ARTEFACTOS GENERADOS."
echo "========================================================"